...
class TransformerEncoderLayer(Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu"):
        ...
        self.linear11=nn.Conv2d(in_dim, in_dim // 8, 1, bias=False)
        self.linear22=nn.Conv2d(in_dim // 8, in_dim, 1, bias=False)
        self.activation11=nn.ReLU(inplace=True)
        self.dropout11=nn.Dropout()
        self.linear33=nn.Conv2d(in_dim, in_dim // 8, 1, bias=False)
        self.linear44=nn.Conv2d(in_dim // 8, in_dim, 1, bias=False)
        self.activation22=nn.ReLU(inplace=True)
        self.dropout22=nn.Dropout()
        self.gamma = nn.Parameter(torch.zeros(1))
    ...
    def forward(self, src: Tensor,srcc: Tensor, src_mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None) -> Tensor:
        ...
        src2 = self.self_attn(src, srcc, srcc, attn_mask=src_mask,
                               key_padding_mask=src_key_padding_mask)[0]

        src3 = src + self.dropout1(src2)
        src3 = self.norm1(src3)
        
        b,c,s=src2.permute(1,2,0).size()
        w_a = self.linear22(self.dropout11(self.activation11(self.linear11(self.avg_pool(src2.view(b,c,int(s**0.5),int(s**0.5)))))))
        w_m = self.linear44(self.dropout22(self.activation22(self.linear33(self.max_pool(src2.view(b,c,int(s**0.5),int(s**0.5)))))))

        ww = self.conv(torch.cat((w_a,w_m),1))
        src4 = (src3.view(b,c,int(s**0.5),int(s**0.5))+self.gamma*ww*src3.view(b,c,int(s**0.5),int(s**0.5))).view(b,c,-1).permute(2, 0, 1)
        src5 = self.linear2(self.dropout(self.activation(self.linear1(src4))))
        src6 = src4 + self.dropout2(src5)
        src6 = self.norm2(src6)
        return src6
